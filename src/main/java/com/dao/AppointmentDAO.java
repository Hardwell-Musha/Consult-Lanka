...
public class AppointmentDAO {
	...
	public boolean addAppointment(Appointment ap) {
		...
		try {
			...
			String sql = "insert into appointment(user_id,fullname,gender,age,appoint_date,email,phno,reason,consultant_id,address,status) values(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<Appointment> getAllAppointmentByConsultantLogin(int consultantId) {
		...
		try {
			...
			String sql = "select * from appointment where consultant_id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
	...
	public boolean updateCommentStatus(int id, int consultId, String comm) {
		boolean f = false;
		try {
			String sql = "update appointment set status=? where id=? and consultant_id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, comm);
			ps.setInt(2, id);
